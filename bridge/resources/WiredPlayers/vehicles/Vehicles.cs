...
namespace WiredPlayers.vehicles
{
    public class Vehicles : Script
    {
        ...
        public static bool hasPlayerVehicleKeys(Client player, NetHandle vehicle)
        {
            bool hasKeys = false;
        ...
        private bool isVehicleTrunkInUse(NetHandle vehicle)
        {
            ...
            foreach (Client player in NAPI.Pools.GetAllPlayers())
            {
                ...
                {
                    NetHandle openedVehicle = NAPI.Data.GetEntityData(player, EntityData.PLAYER_OPENED_TRUNK);
        ...
        private void OnPlayerDisconnectedHandler(Client player, byte type, string reason)
        {
            if (NAPI.Data.HasEntityData(player, EntityData.PLAYER_PLAYING) == true)
            {
                ...
                if (gasTimerList.TryGetValue(playerId, out gasTimer) == true)
                {
                    // Eliminamos el timer
                    gasTimer.Dispose();
        ...
        {
            if (NAPI.Entity.GetEntityType(entity) == EntityType.Player && NAPI.Data.HasEntityData(entity, EntityData.PLAYER_PARKED_VEHICLE) == true)
            {
                // Sacamos el jugador
        ...
        private void OnPlayerEnterVehicle(Client player, NetHandle vehicle, sbyte seat)
        {
            if (Convert.ToInt32(seat) == Constants.VEHICLE_SEAT_DRIVER)
            {
                if (NAPI.Data.HasEntityData(vehicle, EntityData.VEHICLE_TESTING) == true)
                {
                    ...
                    {
                        NetHandle testingVehicle = NAPI.Data.GetEntityData(player, EntityData.PLAYER_TESTING_VEHICLE);
        ...
        private void OnVehicleDeathTimer(object vehicleObject)
        {
            try
            {
                ...
                else
                {
                    ...
                    NetHandle newVehicle = NAPI.Vehicle.CreateVehicle(NAPI.Util.VehicleNameToModel(vehicleModel.model), vehicleModel.position, vehicleModel.rotation.Z, new Color(0, 0, 0), new Color(0, 0, 0));
        ...
        [Command("cinturon")]
        public void toggleSeatbelt(Client player)
        {
        ...
        [Command("capo")]
        public void openHoodCommand(Client player)
        {
            if (NAPI.Data.GetEntityData(player, EntityData.PLAYER_KILLED) != 0)
            {
                NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_ERROR + Messages.ERR_PLAYER_IS_DEATH);
        ...
        public void openTrunkCommand(Client player, String action)
        {
            if (NAPI.Data.GetEntityData(player, EntityData.PLAYER_KILLED) != 0)
            {
                NAPI.Chat.SendChatMessageToPlayer(player, Constants.COLOR_ERROR + Messages.ERR_PLAYER_IS_DEATH);
                        ...
                        case "guardar":
                            ...
                            else
                            {
                                ...
                                else if (NAPI.Data.HasEntityData(player, EntityData.PLAYER_RIGHT_HAND) == true)
                                {
                                    ...
                                    if (Int32.TryParse(rightHand.hash, out itemHash) == false)
                                    {
